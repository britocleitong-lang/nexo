...
import {
  ...
  alternarAtiva, ocorrenciasPendentes, ocorrenciasFuturas, materializar, pularProxima,
...
export function RecorrenciasPage() {
  ...
  const [futuras, setFuturas] = useState<OcorrenciaPendente[]>([]);
  ...
  function recarregar() {
    ...
    setFuturas(ocorrenciasFuturas(60));
  ...
  return (
    <div>
      <PageHeader
        title="Recorrências"
        subtitle="O que se repete todo mês — salário, aluguel, assinaturas. O Nexo repõe os lançamentos que faltarem."
        ...
        {recorrencias.length === 0 ? (
          <Card>
            <EmptyState
              ...
              action={!somenteLeitura && <Button variant="primary" icon={<Plus size={16} />} onClick={abrirNovo}>Nova recorrência</Button>}
            />
          </Card>
        ) : (
          <Card>
      ...
      {futuras.length > 0 && (
        <div className="section">
          <h2 className="section-title">Previsto para os próximos 60 dias</h2>
          <Card>
            <div className="list">
              {futuras.slice(0, 15).map((o, i) => (
                <div key={`${o.recorrencia.id}-${o.data}-${i}`} className="list-row">
                  <div className="list-row-main">
                    <div className="list-row-title">{o.recorrencia.descricao}</div>
                    <div className="list-row-meta">{formatarData(o.data)} · {textoPrazo(diasRestantes(o.data))}</div>
                  </div>
                  <div className={`list-row-value tabular ${o.recorrencia.tipo}`}>
                    {o.recorrencia.tipo === "receita" ? "+" : "−"}{formatarMoeda(o.recorrencia.valor)}
                  </div>
                </div>
              ))}
